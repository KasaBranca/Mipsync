...
bool EmitTexturesDataC(const std::vector<std::string>& projectRelativePaths,
    ...
    try {
        ...
        for (size_t i = 0; i < projectRelativePaths.size(); ++i) {
            ...
            MIPSYNC_INFO("PS1 texture export: {} -> {}x{}", rel, loaded.back().tex.width,
                         loaded.back().tex.height);
        }
            ...
            if (backgroundSet.find(rel) != backgroundSet.end()) {
                ...
                if (hasBackgrounds) {
                    if (slot == 0) {
                        ...
                        vramX = 512 + (lowerSlot % 4) * 128;
                        vramY = 256 + (lowerSlot / 4) * 128;
                    ...
                    if (slot >= 4)
                        slot += 1; // Skip slot 4, which overlaps the framebuffer edge.
                    vramX = 512 + (slot % 4) * 128;
                    vramY = (slot / 4) * 256;
                }
                normalTexCount++;
            }
