...
### Versioning
- Engine + Hub releases use **`YYYY.M.D` date stamps** (today: `2026.5.29`).
...
### Highlights
- **Stale `out/` artefacts no longer mask new builds.** Ps1Build now
  wipes `out/PSX.EXE`, `out/game.cue`, `out/game.bin`, and
  `out/SYSTEM.CNF` at the start of every build. Without this, a leftover
  PSX.EXE from a previous engine release made the SDK-success check return
  a false positive 窶・the prebuilt fallback never re-staged the freshly
  bundled starter, so the emulator booted the old PS-EXE (visible in the
  OpenBIOS log as a stale `PC0` value) and stayed black.
- **Starter PSX.EXE no longer boots to a black screen.** `main.c` now uses
  proper PSn00bSDK double-buffering; `draw` was previously fixed at VRAM
  `y=240` while `disp` stayed at `y=0` and never swapped, so all clears
  + glyphs went off-screen.
